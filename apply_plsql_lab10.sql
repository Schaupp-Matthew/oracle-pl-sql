...
CREATE OR REPLACE
  TYPE BODY base_t IS
    ...
    /* Formalized default constructor. */  -- Must disallow any other value except a null, 'NEW', or 'OLD'
    CONSTRUCTOR FUNCTION base_t            -- for the oname parameter.
    ( oname  VARCHAR2
    , name   VARCHAR2 ) RETURN SELF AS RESULT IS
    BEGIN
      /* Assign an oname value. */
      IF(name = NULL OR name = 'NEW' OR name = 'OLD') THEN
        self.oname := oname;
      END IF;
...
-- --------------------------------------------------
-- Test ability to create an instance of base_t
-- --------------------------------------------------
DECLARE
...
-- Insert a row into the logger table with the default no parameter constructor
INSERT INTO logger
VALUES (logger_s.NEXTVAL, base_t());

-- Inser a row into the logger table with the parameter-driven constructor
DECLARE
...
-- Test for inserted rows into the logger table
...
--  , OVERRIDING MEMBER FUNCTION get_oname RETURN VARCHAR2
--  , OVERRIDING MEMBER FUNCTION set_oname(oname VARCHAR2)
...
-- Create a contact_t subtype of base_t
CREATE OR REPLACE
...
-- Inser a row into the logger table with the parameter-driven constructor for item_t
DECLARE
  /* Declare a variable of the UDT type. */
  lv_item  ITEM_T;
...
BEGIN
  ...
  lv_item := item_t(
      oname               => 'ITEM_T'
    , name                => 'NEW'
    , item_id             => 100 --c.item_id
    , item_barcode        => 'barcode' --c.item_barcode
    , item_type           => 100 --c.item_type
    , item_title          => 'Title' --c.item_title
    , item_subtitle       => 'Subtitle' --c.item_subtitle
    , item_rating         => 'Rating' --c.item_rating
    , item_rating_agency  => 'Agen' --c.item_rating_agency
    , item_release_date   => SYSDATE --c.item_release_date
    , created_by          => 1 --c.created_by
    , creation_date       => SYSDATE --c.creation_date
    , last_updated_by     => 1 --c.last_updated_by
    , last_update_date    => SYSDATE ); --c.last_update_date 
 
    /* Insert instance of the item_t object type. */
    INSERT INTO logger
...
-- Inser a row into the logger table with the parameter-driven constructor for contact_t
DECLARE
  /* Declare a variable of the UDT type. */
